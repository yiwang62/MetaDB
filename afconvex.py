    ...
    if len(kpoints)!=0 and not kpo:
      for kpoint in kpoints:
        try:
          ...
          pass
        if kpo:
          urllib.request.urlretrieve(entry+'/'+kpoint, os.path.join(posdir, spname + ".KPOINTS"))
          break
    ...
    if len(contcars)!=0 and not con:
      for contcar in contcars:
        ...
        if con:
          ...
          try:
            #print (alines[5])
            if alines[5].find(ff["elements"][0])<0:
              alines.insert(5,"   "+ff["elements"]+'\n')
          except:
            pass
          N = 8+sum(ff["composition"])
          open(poscarfile, 'w').writelines(alines[:N])
          outputfiles(poscarfile, __exfilelist, OUTDIR)
          break

  open(os.path.join(posdir, spname + ".POTCAR"), 'w').write(str(species_pp)+'\n')
  open(os.path.join(posdir, spname + ".ENCUT"), 'w').write(str(energy_cutoff)+'\n')
...
def outMPdata(posdir, _dstack, calhull, ehull):
  ...
  print ("Compound         Formula           SG Symmetry     Pearson Prototype         Tenergy(eVoutdiratom)    Fenergy : Primitive POSCAR")
  dstack = []
  for ff in _dstack:
  ...
  for ff in dstack:
    ...
    for cc in KPOINTS:
      try:
        entry.files[cc](os.path.join(posdir,spname + ".KPOINTS"))
      except:
        ...
        urllib.request.urlretrieve(kpoints, os.path.join(posdir,spname + ".KPOINTS"))
      break
    ...
    for cc in CONTCARS:
      ...
      with open(os.path.join(posdir, spname + ".VASP"), 'w') as f0:
        for l in range(0,5):
          f0.write("{}\n".format(lines[l]))
        f0.write("   {}\n".format(ff["elements"]))
        natom = sum(ff["composition"])
        lstart = 6
        if lines[5].find(ff["elements"][0])<0:
          lstart = 5
        for l in range(lstart,lstart+2+natom):
          f0.write("{}\n".format(lines[l]))
        print(os.path.join(posdir, spname + ".VASP"))
        found = True
      outputfiles(os.path.join(posdir, spname + ".VASP"), __exfilelist, OUTDIR)
      break
