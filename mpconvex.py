...
def ExcelPhaseName():
  ...
  #print ('eeeeeeeeeeee', path)
  df = pd.read_excel(os.path.join(path,'2019-02-08-List-Phases.xlsx'), sheet_name='model_MP')
...
def outMPdata(posdir, _dstack, calhull, ehull):
  ...
  for ff in dstack:
    ...
    if len(out) < 2: out = (pname.split('#')[0]).split('|')
    ...
    if calhull:
      spname = os.path.join(posdir,ff["phases"])
    else:
      spname = os.path.join(posdir,ff["phasename"].replace('|','_'))
    print(spname+".VASP")
    with open(spname+".VASP",'w') as f0:
      f0.write("{}\n".format(ff["POSCAR"]))
    if calhull:
      outputfiles(spname+".VASP", __exfilelist, OUTDIR)
  ...
  for ff in _dstack:
    ...
    if len(els)==1:
      if els[0] in suspend:
        continue
    dstack.append(ff)
    Phases.append(ff["phasename"]+'+'+ff["pretty_formula"])
...
def calcphasedecompostion(dstack,formula,elist,clist):
  ...
  for x in X:
    ...
    #print (Phases[x[0]])
    print('{:6.2f}  {:12s}  {:12s}  {:12s}  {}'.format(x[1]*100, phases[5], phases[1], phases[2], phases[0]))
    if pcom == "":
      pcom = '{:6.2f}*{}'.format(x[1]*100, phases[5])
    else:
      pcom += ' +' + '{:6.2f}*{}'.format(x[1]*100, phases[5])
