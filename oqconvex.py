...
def downloadsetting(setting,pos,formula):
  ...
  outname = os.path.join(posdir,formula+'+oq-'+pos+".PAR")
  f = open(outname, 'w')
  ...
  for bb,bline in enumerate(blines):
    ...
    if line.startswith('<a href="/materials/export/primitive/poscar/'):
      ...
      if cell=='Primitive':
        ...
        outname = os.path.join(posdir,formula+'+oq-'+pos+".VASP")
        with open(outname, 'w') as f:
          ...
          for ii,line in enumerate(lines):
            ...
            """
            """
        outputfiles(outname,__exfilelist,OUTDIR)
      else:
        urlpos = 'http://oqmd.org/materials/export/conventional/poscar/'+pos
        urllib.request.urlretrieve(urlpos, os.path.join(posdir,pos+".VASP"))
      ...
      if static.find('static</a></td>')>=0:
        ...
        msg = "static"
        break
      elif static.find('standard</a></td>')>=0:
        energy = blines[bb+2].decode("utf-8").strip().replace('<td>','').replace('</td>','')
        setting = line.replace('<tr class=clickableRow href="','http://oqmd.org/').split('"')[0]
        downloadsetting(setting,pos,formula)
        msg = "standard"
      elif static.find('static_lda</a></td>')>=0:
        energy = blines[bb+2].decode("utf-8").strip().replace('<td>','').replace('</td>','')
        setting = line.replace('<tr class=clickableRow href="','http://oqmd.org/').split('"')[0]
        downloadsetting(setting,pos,formula)
        msg = "static_lda"
        break
  print(' {:>8s} {:>13s} : {}'.format(energy,msg,outname))
  return energy
...
def downloadtotalenergy(entry,formula,materialsid):
  ...
  for bb,bline in enumerate(blines):
    ...
    if line.startswith('<a href="/materials/export/primitive/poscar/'):
      ...
      if cell=='Primitive':
        urlpos = 'http://oqmd.org/materials/export/primitive/poscar/'+pos
        outname = os.path.join(posdir,formula+'_oq-'+pos+".VASP")
    elif line.startswith('<tr class=clickableRow href="/analysis/calculation/'):
      static = blines[bb+1].decode("utf-8").strip()
...
while (count < len(sys.argv)):
  if (sys.argv[count] == "-within"):
    ...
    input_contain = False
    input_wCom = False
  elif (sys.argv[count] == "-outdir"):
    count = count + 1
    if (count >= len(sys.argv)):
      break
    OUTDIR = sys.argv[count]
...
from utlis import exfilelist, outputfiles, setoutdir
__exfilelist = exfilelist(OUTDIR)
...
PureElement = "Compounds at this composition"
...
for bidx,bline in enumerate(blines):
  ...
  if line.find(Stable)!=-1:
    ...
    print("Compound  Symmetry Fenergy(eV/atom) Prototype           Cell_Size    Ehull  Tenergy Configuration :",cell, "POSCAR\n")
    dld = True != alldata
    irange = 6
    istart = 0
    continue
  if line.find(Allphases)!=-1 or line.find(PureElement)!=-1:
    print("\n ",Allphases,"\n")
    #dld = False
    dld = True == alldata
    istart = 1
  ...
  if line.startswith('<tr class=clickableRow href="/materials/entry/'):
    ...
    for i in range(irange):
      bb = blines[bidx+1+ii].decode("utf-8").strip()
      ii += 1
      while bb=="": 
        bb = blines[bidx+1+ii].decode("utf-8").strip()
        ii += 1
      bb = bb.replace("<td>","").replace("</td>","").replace("<sub>","").replace("</sub>","")
      out.append(bb)
    materialid = out[0]
    out = out[istart:]
    formula = out[0]
    punitcell= punitcell_formula(out[0],out[5])
    els,com = formula2composition(formula)
    sys.stdout.write('{:<16s} {:<10s} {:>7s} {:<24s} {:>4s} {:>8s}'.format(punitcell,out[1],out[2],out[4], out[5], out[3]))
