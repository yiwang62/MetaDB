...
class MyPhaseClass:
  ...
  def PhaseNameByFileName(self, filename):
    xx = filename.split('+')
    pname = self.PhaseName(xx[0], xx[1])
    if pname==None:
      return 't-'+filename
    else:
      return pname+'+'+xx[-1]

  def PhaseName(self, formula, spacegroup, siteoc=[]):
...
if __name__ == '__main__':
  ...
  for x in os.listdir(test0):
    xx = x.split('+')
    if len(xx) < 2: 
      pname = None
    else:
      pname = myphaseclass.PhaseNameByFileName(x)
    if pname == None: pname=x
    shutil.copy2(test0+x,test1+pname)
